...
var translation = [-150, 0, -360]; //x-y-z of F
var rotation = [0, 0, 0];
...
window.onload = function init()
{
	...
	//update FOV
	var angleFOVValue = document.getElementById("FOVvalue");
	angleFOVValue.innerHTML = angleFOV;
	document.getElementById("sliderFOV").onchange = function(event) {		
		angleFOVValue.innerHTML = event.target.value;
		FOV_Radians = degToRad(event.target.value);
		render();
    };
...
function render() 
{
	...
	matrix = m4.perspective(FOV_Radians, aspect, zNear, zFar);
    //matrix = m4.projection(gl.canvas.clientWidth, gl.canvas.clientHeight, 400); //not needed cause its include in perspective
